...
def test_delay_seconds():
    ...
    result = parse("python -m http.server")  # Changed: delays are now part of dependencies
    expected = ParsedCommand(None, None, [], [], ["python", "-m", "http.server"])
    assert result == expected, f"Expected {expected}, got {result}"
...
def test_named_with_delay():
    """Test parsing a named command - delays now handled via dependencies"""
    result = parse("A=python -m http.server")
    expected = ParsedCommand("A", None, [], [], ["python", "-m", "http.server"])
    assert result == expected, f"Expected {expected}, got {result}"
...
def test_complex_command():
    ...
    expected = ParsedCommand("A", None, [], ["silent", "end"], ["python", "-m", "http.server"])
    assert result == expected, f"Expected {expected}, got {result}"
...
def test_benchmark_example():
    ...
    result = parse("|end=ab -n1000 http://localhost:8000/")  # Delay removed - now handled by dependencies
    expected = ParsedCommand(None, None, [], ["end"], ["ab", "-n1000", "http://localhost:8000/"])
    assert result == expected, f"Expected {expected}, got {result}"
...
def test_sequential_example():
    ...
    result = parse(":A=ab -n1000 http://localhost:8000/")  # :A means wait for A to complete
    expected = ParsedCommand(None, None, [parse("A=dummy").dependencies[0] if parse("A=dummy").dependencies else type('MockDep', (), {'key': 'A', 'wait_for_start': False, 'delays': []})()], [], ["ab", "-n1000", "http://localhost:8000/"])
