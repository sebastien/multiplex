...
def test_start_dependency_timing():
	...
	cmd_b = runner.run(parsed.command, key="B", dependencies=parsed.dependencies)
	...
	assert time_diff < 0.5, (
		f"B took too long to start ({time_diff:.3f}s), suggesting it waited for A to end"
	)
...
def test_start_vs_end_dependency():
	...
	cmd_b_end = runner.run(
		parsed_end.command, key="B_END", dependencies=parsed_end.dependencies
	)
	...
	cmd_b_start = runner.run(
		parsed_start.command, key="B_START", dependencies=parsed_start.dependencies
	)
	...
	assert start_dependency_time < end_dependency_time, (
		"Start dependency should be faster than end dependency"
	)
	assert start_dependency_time < 0.5, (
		f"Start dependency took too long ({start_dependency_time:.3f}s)"
	)
